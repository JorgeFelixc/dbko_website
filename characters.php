...
?>
<div id="content">
<div class="top"><img src="skins\top\personagens.png"></a></div>
<div class="mid">
...
if (!empty($_GET['player_id']) && $player->load($_GET['player_id']) || !empty($_GET['player_name']) && $player->find($_GET['player_name'])){
	echo '<hr/><table style="width: 100%"><tr><td><b>Nome:</b> '.htmlspecialchars($player->getAttr('name'))."<br/>\n";
	echo '<b>Level:</b> '.$player->getAttr('level')."<br/>\n";
	echo '<b>Ki Level:</b> '.$player->getAttr('maglevel')."<br/>\n";
	...
	echo '<b>Sexo:</b> '.$gender[$player->getAttr('sex')].'<br/>'."\n";
	if (!empty($cfg['temple'][$player->getAttr('city')]['name']))
		echo "<b>Residencia</b>: ".ucfirst($cfg['temple'][$player->getAttr('city')]['name'])."<br/>";

	if ($player->isAttr('position')){
		echo "<b>Posicao: </b> ".$player->getAttr('position')."<br/>";
	}
	...
	echo "<b>Entrou em:</b> ".$lastlogin."<br/>\n";
	...
	if ($account->load($player->getAttr('account')))
		if (strlen($account->getAttr('comment'))>0){
			echo "<b>Comentarios</b><br/><div style=\"overflow:hidden\"><pre>".htmlspecialchars($account->getAttr('comment'))."</pre></div><br/>\n";
		}	
	echo '<hr/>';
		...
		echo '<b>Mortes</b><br/>';
			foreach ($deaths as $death){
				...
				if ($killer->exists())
					...
					$name = $death['killer'];
				echo '<i>'.date("jS F Y H:i:s",$death['date']).'</i> Morto no Level '.$death['level'].' by '.$name.'<br/>';
			}
		}
	}
}elseif (isset($_GET['player_id']) || isset($_GET['player_name'])) echo 'Personagem nao existe';
?>
